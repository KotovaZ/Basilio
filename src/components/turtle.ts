...
@gravity
@enemy(1)
export class Turtle extends Unit {
    ...
    render = (ctx: CanvasRenderingContext2D) => {
        ...
        const hasXCollision = this.collisions[90] || this.collisions[270];
        if (hasXCollision) {
            ...
            if (this.action === 'die' && collisionObject.hasOwnProperty('enemy')) {
                if (collisionObject.enemy === this.enemy) {
            ...
            if (!this.velocity.x)
                this.velocity.x = this.vector.x * this.speed * 4 / window.framerate;
